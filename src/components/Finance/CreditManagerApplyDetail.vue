...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      back() {
        this.$router.push("/Finance/CreditManagerApply/id=" + window.location.href.split("id=")[1]);
      },
      choosethis(index, id) {
      ...
      save() {
        ...
        this.$http
          .get("api/Web_CreditManager/EditBankManageLoan",{
            params: {
              AgreeProtocol:true,
              ManagerLoanID: this.LoanOrderID,
              Code:this.form.code,
              Token: getCookie("token")
            }
          })
